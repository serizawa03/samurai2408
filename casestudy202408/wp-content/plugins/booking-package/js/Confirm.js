...
/* globals Booking_Package_Elements */
    ...
    Confirm.prototype.dialogPanelShow = function(subject, message, through, level, callback){
        ...
        if (document.getElementById("dialogPanel") == null) {
            ...
            var subjectPanel = dialogPanel.getElementsByClassName("subject")[0];
            subjectPanel.classList.remove('warning');
            if (level === 1) {
                
                subjectPanel.classList.add('warning');
    ...
    function Booking_Package_DatabaseUpdateErrors(errorElement) {
        
        try {
            
            if (navigator.clipboard) {
                
                var queries = errorElement.getElementsByClassName('query');
                for (var i = 0; i < queries.length; i++) {
                    
                    queries[i].onclick = function () {
                ...
                var copy_icons = errorElement.getElementsByClassName('copy_icon');
                for (var i = 0; i < copy_icons.length; i++) {
                    
                    copy_icons[i].onclick = function () {
                        
                        var id = this.id.replace(/^copy_/, '');
